...
def image_to_annotations(img_fn: str, out_dir: str, background_key_color: tuple[int, int, int] = (255, 0, 255),
                          contrast_threshold: float = 100.0) -> None:
    """
    ...
    The character keeps its original drawn colors. If its average color is too close to
    background_key_color (the chroma-key color Unity keys out as transparent — see
    unity_animate.py's CHROMA_KEY_RGB), a contrasting outline is drawn around its silhouette
    so it doesn't get partially keyed out / blend into the background.

    Params:
        img_fn: path to RGB image
        out_dir: directory where outputs will be saved
        background_key_color: the (R, G, B) chroma-key color the exported video is rendered against
        contrast_threshold: max RGB distance to background_key_color before an outline is added
    """
    ...
    # clamp keypoints within the cropped image bounds; the pose estimator can return
    # coordinates that round up to exactly the crop's width/height when the character
    # is near the crop edge, which fails the strict '< width'/'< height' checks downstream
    ...
    # if the character's own color is too close to the chroma-key background color, it would
    # partially blend into the background once keyed out in Unity — add a contrasting outline
    mean_bgr = cv2.mean(cropped, mask=mask)[:3]
    mean_rgb = (mean_bgr[2], mean_bgr[1], mean_bgr[0])
    color_distance = float(np.linalg.norm(np.array(mean_rgb) - np.array(background_key_color)))
    if color_distance < contrast_threshold:
        luminance = 0.299 * mean_rgb[0] + 0.587 * mean_rgb[1] + 0.114 * mean_rgb[2]
        outline_bgr = (0, 0, 0) if luminance > 128 else (255, 255, 255)
        thickness = max(2, round(0.015 * max(cropped.shape[:2])))
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        cv2.drawContours(cropped, contours, -1, outline_bgr, thickness)
